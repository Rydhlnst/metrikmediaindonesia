...
export default function NewStaticPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.message || "Gagal membuat halaman");
      ...
      <div className="w-full flex-1 p-4 sm:p-6 lg:p-8 space-y-6">
        <div className="flex items-center justify-between border-b border-black/5 pb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-foreground">Halaman Statis Baru</h1>
              <p className="text-xs text-muted-foreground">Buat halaman informasi CMS seperti Tentang Kami, Pedoman Siber, dll.</p>
            </div>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 max-w-4xl">
          <Card className="rounded-none border border-black/10 bg-white shadow-2xs">
            <CardContent className="p-6">
              <input
                type="text"
                placeholder="Judul halaman (contoh: Tentang Kami, Pedoman Media Siber)"
                value={title}
          ...
          <Card className="rounded-none border border-black/10 bg-white shadow-2xs">
            ...
            <CardContent className="space-y-4 p-6">
              ...
              <div>
                ...
                <Input
                  placeholder="Judul SEO halaman (opsional)"
                  value={seoTitle}
