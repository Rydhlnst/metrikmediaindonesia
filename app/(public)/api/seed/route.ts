...
const IMAGES = [
  { url: "https://picsum.photos/id/1/1200/675", alt: "Ekonomi digital Indonesia" },
  { url: "https://picsum.photos/id/29/1200/675", alt: "Timnas Indonesia" },
  { url: "https://picsum.photos/id/48/1200/675", alt: "Literasi digital pelajar" },
  { url: "https://picsum.photos/id/59/1200/675", alt: "Festival budaya Nusantara" },
  { url: "https://picsum.photos/id/65/1200/675", alt: "Startup EdTech" },
  { url: "https://picsum.photos/id/76/1200/675", alt: "Jonatan Christie" },
  { url: "https://picsum.photos/id/83/1200/675", alt: "Kurikulum Merdeka" },
  { url: "https://picsum.photos/id/96/1200/675", alt: "Upacara adat Tambi" },
  { url: "https://picsum.photos/id/103/1200/675", alt: "Investasi asing" },
  { url: "https://picsum.photos/id/119/1200/675", alt: "Piala Dunia U-20" },
];
...
export async function POST(request: Request) {
  ...
  try {
    ...
    // Reset: delete all articles and media first
    if (reset) {
      results.push("Resetting database...");
      
      const existingArticles = await payload.find({ collection: "articles", limit: 200 });
      for (const art of existingArticles.docs) {
        await payload.delete({ collection: "articles", id: art.id });
      }
      results.push(`Deleted ${existingArticles.docs.length} articles`);

      const existingMedia = await payload.find({ collection: "media", limit: 200 });
      for (const m of existingMedia.docs) {
        await payload.delete({ collection: "media", id: m.id });
      }
      results.push(`Deleted ${existingMedia.docs.length} media files`);
    }
    ...
    // Download images
    results.push("Downloading images...");
    const mediaItems = [];
    for (let i = 0; i < IMAGES.length; i++) {
      ...
      try {
        ...
        results.push(`Image ${i + 1}/${IMAGES.length} uploaded: ${img.alt}`);
        fs.unlinkSync(filepath);
      } catch (e: any) {
        results.push(`Image ${i + 1} failed: ${e.message}`);
