...
export function SubmissionDetail({ id, editorial = false }: { id: number; editorial?: boolean }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!data.data) throw new Error("Submission unavailable");
  ...
  const review = async (status: string) => {
    setSaving(true);
    try { await requestJson(`/api/submissions/${id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ status, adminNote: note || null }) }); toast.success("Submission review updated"); void load(); }
  ...
  const publish = async () => {
    setSaving(true);
    try { await requestJson(`/api/submissions/${id}/publish`, { method: "POST" }); toast.success("Submission was published as an article"); void load(); }
    catch (error) { toastApiError(error); }
    finally { setSaving(false); }
  };
  ...
  const saveUserRevision = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setSaving(true);
    try { await requestJson(`/api/submissions/${id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ title: submission.title, summary: submission.summary, content: submission.content, featuredImage: submission.featuredImage, videoUrl: submission.videoUrl, sources: submission.sources, submit: true }) }); toast.success("Revision submitted"); void load(); }
    catch (error) { toastApiError(error); }
    finally { setSaving(false); }
  };

  if (loading || !submission) return <p className="p-6 text-sm text-muted-foreground">Loading submission…</p>;
